...
class Image(ImageBase, table=True):
    ...
    created_at: datetime = Field(default_factory=datetime.utcnow, nullable=False, description="创建日期")
    updated_at: datetime = Field(default_factory=datetime.utcnow, nullable=False, sa_column_kwargs={"onupdate": datetime.utcnow}, description="最后更新日期")

    file_metadata: Optional[Dict[str, Any]] = Field(default={}, sa_column=Column(JSON), description="图片文件元数据")

    category_id: uuid.UUID = Field(foreign_key="category.id", index=True, description="所属类别ID")
...
class ImageCreate(SQLModel):
    """创建新图片时，API端点可能接收的元数据 (文件本身是 UploadFile)
       或者服务层用于聚合数据的模型。
    """
    title: Optional[str] = Field(None, max_length=255, description="图片标题，若不提供可由文件名生成")
    description: Optional[str] = Field(None, max_length=500, description="图片描述")
