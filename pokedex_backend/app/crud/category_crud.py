...
async def delete_category(
    *, session: Session, category_id: uuid.UUID
    ...
    if images_in_category:
        # 如果删除了图片记录，则在线程池中执行同步的数据库commit操作
        await run_in_threadpool(session.commit)  # 提交图片删除的事务

    # 在线程池中执行同步的数据库delete操作 (针对类别本身)
    await run_in_threadpool(session.delete, category_to_delete)
    # 在线程池中执行同步的数据库commit操作 (针对类别删除)
    await run_in_threadpool(session.commit)  # 提交类别删除的事务
